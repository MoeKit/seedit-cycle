"use strict";	
	...
	Cycle.prototype.initNullHtml = function(ring){
		...
		var html = replaceHtml({
			"cyclesex": _this.sex,
			"enter": _this.sex == "female" ? '<a href="'+_this.enter+'" class="item"><span class="null-data">前往记录经期</span>没有记录周期数据</a>'
											: '<span class="null-data" style="display: inline-block; color: #115b99;">你的老婆没有填写数据</span>'
	...
	Cycle.prototype.initHtml = function(ring){
		...
		var plan_0 = '<span class="clear-float"></span><div class="mask-'+_this.sex+'-inner" >'+cycleAvatar+'<div class="cycle-text">'+cycleText+'</div><div class=""><span class="showy percent">'+ring.next+'</span>天</div></div>';
		var plan_12 = '<span class="clear-float"></span><div class="mask-'+_this.sex+'-inner">'+cycleAvatar+'<div>受孕指数</div><span class="icon-start"><i></i></span></div>';
	...
	Cycle.prototype.initCss = function(ring){
		...
		// 计算当前选中时的箭头指向的位置
			var dayDeg = Math.floor(360/ring.cycle);
			var statusDegPointer = Math.floor(dayDeg*ring.day - dayDeg*0.5);
